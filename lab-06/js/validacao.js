...
const mensagensDeErro = {
    ...
    preco: {
        ...
        customError: 'O preço digitado não é válido.'
    },
    quantidade: {
        valueMissing: 'O campo de quantidade não pode estar vazio.',
        customError: 'A quantidade deve ser um número válido.'
    }
};
...
function validaTelefone(input) {
    ...
    if (!telefone) {
        mensagem = mensagensDeErro.telefone.valueMissing;
    } else if (telefone.length !== 11) {
        mensagem = 'O telefone deve conter 11 dígitos (DDD + número).';
    } else if (!/^\d{2}/.test(telefone)) {
        mensagem = 'O DDD do telefone não é válido.';
    }
...
function validaInstagram(input) {
    ...
    if (!instagram) {
        ...
        mensagem = mensagensDeErro.instagram.customError;
    }
...
function validaNomeProduto(input) {
    ...
    if (!input.value) {
        mensagem = mensagensDeErro.nomeProduto.valueMissing;
    }
...
function validaPreco(input) {
    ...
    if (!input.value) {
        mensagem = mensagensDeErro.preco.valueMissing;
    } else if (isNaN(input.value.replace(/[^0-9]/g, ''))) {
        mensagem = mensagensDeErro.preco.customError;
    }
...
function validaQuantidade(input) {
    ...
    if (!input.value) {
        mensagem = mensagensDeErro.quantidade.valueMissing;
    } else if (isNaN(input.value)) {
        mensagem = mensagensDeErro.quantidade.customError;
    }
